...
#ifndef _PCP_VISITOR_H_
#define _PCP_VISITOR_H_

#include "pcp.h"

class PcpVisitor
{
  // Default implementation is to call superclass visitor
  // Top level (PcpObject, PcpAnnot, PcpAnnotSet should generally be an error.
  // Default traversal had been implented, but has to be enabled.

 protected:

  // Default implementations of visiting children of container objects
  virtual void visitChildren(PcpArrayType* arrayType);
  virtual void visitChildren(PcpArith* arith);
  virtual void visitChildren(PcpArrayAccess* arrayAccess);
  virtual void visitChildren(PcpCopy* copy);
  virtual void visitChildren(PcpUserStmt* userStmt);
  virtual void visitChildren(PcpSequence* sequence);
  virtual void visitChildren(PcpGuard* guard);
  virtual void visitChildren(PcpLoop* loop);
  virtual void visitChildren(PcpScop* scop);
  virtual void visitChildren(PcpCompare* compare);
