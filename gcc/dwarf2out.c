...
dwarf2out_decl (tree decl)
{
  ...
  switch (TREE_CODE (decl))
   ...
   /* bviyer: I have this if statement for cilk_for and cilk_spawn. For
    * cilk_spawn and cilk_for we cannot create the debug information because
    * those statements are internally generated by the compiler and not 
    * provided by the user, so we cant step through those
    */
  if (context_die != NULL)
    gen_decl_die (decl, NULL, context_die);
}
