...
typedef struct GTY(()) dw_fde_struct {
  ...
  unsigned first_section_p : 1;
  /* True if the function is spread on multiple sections and this FDE
     belongs to the last section.  */
  unsigned last_section_p : 1;
}
dw_fde_node;
...
/* Information regarding a lexical block.
   ...
   .debug_range section.  */
typedef struct GTY(()) block_aux_struct
{
  /* The block number.  */
  int block_num;
...
dwarf2out_switch_text_section (unsigned int i)
{
  ...
  fde = current_fde ();
  gcc_assert (fde && !fde->dw_fde_switched_sections);
...
typedef struct GTY(()) die_sections_def
{
...
{
  var_loc_list *temp = (var_loc_list *) *slot;
  struct label_wrapper *lw = (struct label_wrapper *)label;
  const char *new_label = lw->label;
  ...
  newloc->var_loc_note = temp->last->var_loc_note;
  newloc->end_section_p = false;
  if (strstr (new_label, "SECTIONE") != 0)
    newloc->end_section_p = true;

  /* Add NEWLOC to the end of list and update LAST.  */
  temp->last->next = newloc;
  temp->last = newloc;
  ...
  for (i = 0; i < arange_table_in_use; i++)
      ...
      if (die->die_tag == DW_TAG_subprogram)
	{
	  ...
	  if (!COMP_UNIT_HAS_SECTIONS
	      || !die->sections_info.more_than_one_sec_p)
	    {
	  ...
	  else
	    {
		 ...
		 which mark the range of each section.  */
	      unsigned size, i = 0;
	      size = VEC_length (const_str,
				 die->sections_info.section_start_labels);

	      for (i = 0; i < size; i++)
		{
		  const char *tmps
		    = VEC_index (const_str,
				 die->sections_info.section_start_labels, i);
                  const char *tmpe
		    = VEC_index (const_str,
				 die->sections_info.section_end_labels, i);
...
/* Convert the CFI instructions for the current function into a
   location list.  The current function has multiple sections.  This is
  ...
  if (declaration)
	...
	equate_decl_number_to_die (decl, subr_die);
	  ...
	  if (crtl->subsections.number_of_sections == 1)
		...
		{
		  tmps = VEC_index (const_str,
				    crtl->subsections.section_start_labels, i);
		  ...
		  /* Record the arange of the function.  In order to
...
dwarf2out_finish (const char *filename)
{
  ...
  if (block_labels_table)
    {
      htab_traverse (block_labels_table, clear_block_labels, NULL);
