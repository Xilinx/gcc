...
{
  vec_safe_reserve_exact (v, len - vec_safe_length (v) PASS_MEM_STAT);
...
{
  vec_safe_reserve_exact (v, len - vec_safe_length (v) PASS_MEM_STAT);
  v->quick_grow_cleared (len PASS_MEM_STAT);
}
...
{
  gcc_checking_assert (len > 0 && length () <= len && len <= pfx_.alloc_);
  pfx_.num_ = len;
}
...
{
  gcc_checking_assert (len > 0 && length () <= len);
  reserve_exact (len - length () PASS_MEM_STAT);
  vec_->quick_grow (len);
}
