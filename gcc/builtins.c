		...
		int ignore)
{
  ...
  switch (fcode)
    ...
    case BUILT_IN_CILK_ENTER_BEGIN:
      target = expand_builtin_cilk_metadata ("cilk_enter_begin", exp);
      if (target)
	return target;
      break;

    case BUILT_IN_CILK_ENTER_H_BEGIN:
      target = expand_builtin_cilk_metadata ("cilk_enter_helper_begin", exp);
      if (target)
	return target;
      break;

    case BUILT_IN_CILK_ENTER_END:
      target = expand_builtin_cilk_metadata ("cilk_enter_end", exp);
      if (target)
	return target;
      break;

    case BUILT_IN_CILK_SPAWN_PREPARE:
      target = expand_builtin_cilk_metadata ("cilk_spawn_prepare", exp);
      if (target)
	return target;
      break;

    case BUILT_IN_SPAWN_OR_CONT:
      target = expand_builtin_cilk_metadata ("cilk_spawn_or_continue", exp);
      if (target)
	return target;
      break;

    case BUILT_IN_CILK_DETACH_BEGIN:
      target = expand_builtin_cilk_metadata ("cilk_detach_begin", exp);
      if (target)
	return target;
      break;

    case BUILT_IN_CILK_DETACH_END:
      target = expand_builtin_cilk_metadata ("cilk_detach_end", exp);
      if (target)
	return target;
      break;

    case BUILT_IN_CILK_SYNC_BEGIN:
      target = expand_builtin_cilk_metadata ("cilk_sync_begin", exp);
      if (target)
	return target;
      break;

    case BUILT_IN_CILK_SYNC_END:
      target = expand_builtin_cilk_metadata ("cilk_sync_end", exp);
      if (target)
	return target;
      break;

    case BUILT_IN_CILK_LEAVE_BEGIN:
      target = expand_builtin_cilk_metadata ("cilk_leave_begin", exp);
      if (target)
	return target;
      break;

    case BUILT_IN_CILK_LEAVE_END:
      target = expand_builtin_cilk_metadata ("cilk_leave_end", exp);
      if (target)
	return target;
      break;

    case BUILT_IN_CILK_RESUME:
      target = expand_builtin_cilk_metadata ("cilk_resume", exp);
      if (target)
	return target;
      break;

    case BUILT_IN_LEAVE_STOLEN:
      target = expand_builtin_cilk_metadata ("cilk_leave_stolen", exp);
      if (target)
	return target;
      break;

    case BUILT_IN_SYNC_ABANDON:
      target = expand_builtin_cilk_metadata ("cilk_sync_abandon", exp);
      if (target)
	return target;
      break;
	...
	("cilkscreen_disable_instrumentation", exp);
      if (target)
	return target;
      break;
    ...
    case BUILT_IN_CILKSCREEN_EN_INSTR:
      target = expand_builtin_cilk_metadata
	("cilkscreen_enable_instrumentation", exp);
      if (target)
	return target;
      break;
    ...
    case BUILT_IN_CILKSCREEN_AQUIRE_LOCK:
      target = expand_builtin_cilk_metadata ("cilkscreen_aquire_lock", exp);
      if (target)
	return target;
      break;

    case BUILT_IN_CILKSCREEN_REL_LOCK:
      target = expand_builtin_cilk_metadata ("cilkscreen_release_lock", exp);
      if (target)
	return target;
      break;

    case BUILT_IN_CILKSCREEN_METACALL:
      target = expand_builtin_cilk_metadata ("cilkscreen_metacall", exp);
      if (target)
	return target;
