...
static vec<tree, va_gc> *c_parser_elem_fn_expr_list (c_parser *parser);
			       ...
			       tree *objc_foreach_object_declaration)
{
  ...
  while (true)
      ...
      if (nested)
	{
	  ...
	  /* DECL_SAVED_TREE info. must be pushed into stmt_list_stack.  */
	  if (flag_enable_cilk && DECL_SAVED_TREE (current_function_decl))
	    vec_safe_push (stmt_list_stack, 
      ...
      else
	{
	  ...
	  /* DECL_SAVED_TREE info must be pusehd into stmt_list stack.  */
	  if (flag_enable_cilk && DECL_SAVED_TREE (current_function_decl))
	    vec_safe_push (stmt_list_stack,
			   DECL_SAVED_TREE (current_function_decl));
				  ...
				  struct c_declarator *inner)
{
  ...
  if (c_parser_next_token_is (parser, CPP_OPEN_SQUARE))
      ...
      else
	{
      ...
      if (c_parser_next_token_is (parser, CPP_CLOSE_SQUARE))
	c_parser_consume_token (parser);
      else if (flag_enable_cilk && c_parser_next_token_is (parser, CPP_COLON))
	{
	  error_at (c_parser_peek_token (parser)->location,
		    "array notations cannot be used in declaration.");
...
c_parser_compound_statement (c_parser *parser)
{
  ...
  if (flag_enable_cilk && contains_array_notation_expr (stmt))
...
c_parser_statement_after_labels (c_parser *parser)
{
  ...
  switch (c_parser_peek_token (parser)->type)
	...
	case RID_CILK_SYNC:
	  c_parser_consume_token (parser);
	  if (!flag_enable_cilk)
	    fatal_error ("-fcilkplus must be enabled to use %<cilk_sync%>");
	  else
	    {
...
c_parser_if_statement (c_parser *parser)
{
  ...
  if (flag_enable_cilk && contains_array_notation_expr (if_stmt))
...
c_parser_unary_expression (c_parser *parser)
{
  ...
  switch (c_parser_peek_token (parser)->type)
    ...
    case CPP_PLUS_PLUS:
      ...
      if (flag_enable_cilk && TREE_CODE (op.value) == ARRAY_NOTATION_REF)
  ...
  while (true)
	...
	case CPP_OPEN_SQUARE:
	  ...
	  if (flag_enable_cilk
		...
		{	
		  /* This is where they are going to find the values 
	...
	case CPP_PLUS_PLUS:
	  /* Postincrement.  */
	  c_parser_consume_token (parser);
	  if (flag_enable_cilk && TREE_CODE (expr.value) == ARRAY_NOTATION_REF)
...
c_parser_elem_fn_processor_clause (c_parser *parser)
{
  ...
  if (!c_parser_next_token_is (parser, CPP_CLOSE_PAREN))
	...
	{
	  c_parser_consume_token (parser);
	  vec_safe_push (proc_vec_list, build_string (strlen ("pentium_4"), 
						      "pentium_4"));
	}
	...
	{
	  c_parser_consume_token (parser);
	  vec_safe_push (proc_vec_list, build_string (strlen ("core2_duo_sse3"),
						      "core2_duo_sse3"));
	}
      else if (token->value && TREE_CODE (token->value) == IDENTIFIER_NODE
	       && simple_cst_equal (token->value,
