...
#@#@# 
#@#@# It has been AutoGen-ed  July 30, 2012 at 06:29:51 PM by AutoGen 5.12
...
if [ ! -f $meltcheckruntime_stamp -o $meltcheckruntime_stamp -ot "$GCCMELT_RUNTIME_ARGS" \
   ...
   meltbuild_info melt-build-script.tpl:673/592 $meltcheckruntime_args  is
   cat $meltcheckruntime_args > /dev/stderr
    $GCCMELT_CC1_PREFIX $GCCMELT_CC1 @$meltcheckruntime_args \
	|| meltbuild_error melt-build-script.tpl:676/593 failed with arguments @$meltcheckruntime_args
    ...
    meltbuild_info melt-build-script.tpl:684/594 done check runtime  $meltcheckruntime_stamp
else
    meltbuild_info melt-build-script.tpl:686/595 keeping runtime checks  $meltcheckruntime_stamp
fi

if [ "$melt_overall_goal" = "checkruntime" ]; then
    meltbuild_info melt-build-script.tpl:690/596 done checkruntime overall goal with stamp  $melt_final_translator_stamp
    exit 0
fi
################################################################
################################################################
#@ melt-build-script.tpl:695/597
if [ "$melt_overall_goal" = "regenerate" ]; then
    meltbuild_info melt-build-script.tpl:697/598 regenerating runtime support
    ...
    echo ' -DGCCMELT_FROM_ARG="melt-build-script.tpl:701/599"' > $meltregen_argstemp
   ...
   meltbuild_info melt-build-script.tpl:711/600 $meltregen_args  is
   cat $meltregen_args > /dev/stderr
    $GCCMELT_CC1_PREFIX $GCCMELT_CC1 @$meltregen_args \
	|| meltbuild_error melt-build-script.tpl:714/601 failed with arguments @$meltregen_args
    meltbuild_info melt-build-script.tpl:715/602 done regenerate overall goal
    exit 0
fi

################################################################
### the generated documentation meltgendoc.texi melt-build-script.tpl:720/603
...
if [   ! -f meltgendoc.texi  -o meltbuild-sources/warmelt-first.melt -nt meltgendoc.texi  \
 ...
 -o meltbuild-sources/xtramelt-opengpu.melt -nt meltgendoc.texi   ]; then
   meltbuild_info melt-build-script.tpl:724/604 generating meltgendoc.texi
   meltgen_args=meltbuild-gendoc.args-tmp$$
   echo ' -DGCCMELT_FROM_ARG="melt-build-script.tpl:726/605"' > $meltgen_args
   ...
   meltbuild_info melt-build-script.tpl:738/606  meltbuild-gendoc.args is
   cat meltbuild-gendoc.args > /dev/stderr
   $GCCMELT_CC1_PREFIX $GCCMELT_CC1 @meltbuild-gendoc.args \
     || meltbuild_error melt-build-script.tpl:741/607 failed with arguments @meltbuild-gendoc.args
else
   meltbuild_info melt-build-script.tpl:743/608 keeping meltgendoc.texi
fi

################
meltbuild_info melt-build-script.tpl:747/609 successfully done with times at `date '+%x %H:%M:%S'`: ; times > /dev/stderr

################################################################
#@ melt-build-script.tpl:750/610
if [ "$melt_overall_goal" = "gendoc" ]; then
    meltbuild_info melt-build-script.tpl:752/611 done gendoc overall goal with stamp  $melt_final_translator_stamp
    exit 0
fi

## #@ melt-build-script.tpl:756/612 if we get here something is wrong in this script
meltbuild_error  melt-build-script.tpl:757/613 unexpected MELT overall goal "$melt_overall_goal" buggy melt-build-script.tpl
#@ eof melt-build-script.tpl:758/614 end of generated melt-build-script.sh
