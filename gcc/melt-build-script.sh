...
#@#@# 
#@#@# It has been AutoGen-ed  September  6, 2012 at 04:22:53 PM by AutoGen 5.16
...
if [ ! -f $meltcheckruntime_stamp -o $meltcheckruntime_stamp -ot "$GCCMELT_RUNTIME_ARGS" \
    ...
    echo ' -DGCCMELT_FROM_ARG="melt-build-script.tpl:683/597"' > $meltcheckruntime_argstemp
    ...
    [ -f "$meltcheckruntime_args" ] || meltbuild_error  melt-build-script.tpl:694/598 missing check runtime args  "$meltcheckruntime_args"
   meltbuild_info melt-build-script.tpl:695/599 $meltcheckruntime_args  is
   ...
   if [ -n "$MELTGCCBUILTIN_BUILD_WITH_CXX" ]; then
	   ...
	   || meltbuild_error melt-build-script.tpl:699/600 failed  $GCCMELT_CC1PLUS with arguments @$meltcheckruntime_args
   else
       $GCCMELT_CC1_PREFIX $GCCMELT_CC1 @$meltcheckruntime_args \
	   || meltbuild_error melt-build-script.tpl:702/601 failed  $GCCMELT_CC1 with arguments @$meltcheckruntime_args
   fi
   meltbuild_info melt-build-script.tpl:704/602 done check runtime with $meltcheckruntime_args
   #@ melt-build-script.tpl:705/603 checkhello
    meltcheckhelloworld_args=meltbuild-checkhelloworld.args 
    meltcheckhelloworld_argstemp=$meltcheckhelloworld_args-tmp$$
    echo ' -DGCCMELT_FROM_ARG="melt-build-script.tpl:708/604"' > $meltcheckhelloworld_argstemp
    ...
    [ -f "$meltcheckhelloworld_args" ] || meltbuild_error  melt-build-script.tpl:722/605 missing check helloworld args  "$meltcheckhelloworld_args"
   meltbuild_info melt-build-script.tpl:723/606 $meltcheckhelloworld_args  is
   cat $meltcheckhelloworld_args < /dev/null > /dev/stderr
    $GCCMELT_CC1_PREFIX $GCCMELT_CC1 @$meltcheckhelloworld_args \
	|| meltbuild_error melt-build-script.tpl:726/607 running helloworld failed with arguments @$meltcheckhelloworld_args
   meltbuild_info melt-build-script.tpl:727/608 done check helloworld with $meltcheckhelloworld_args
   #@ melt-build-script.tpl:728/609 runtime stamp
    meltcheckruntime_stamptemp=$meltcheckruntime_stamp-tmp$$
    [ -f "$GCCMELT_RUNTIME_C" ] || meltbuild_error melt-build-script.tpl:730/610 missing MELT runtime C file $GCCMELT_RUNTIME_C
    ...
    [ -f "$melt_final_translator_stamp" ] || meltbuild_error melt-build-script.tpl:735/611 missing final translator stamp "$melt_final_translator_stamp"
    [ -f "$melt_final_application_stamp" ] || meltbuild_error melt-build-script.tpl:736/612 missing final application stamp "$melt_final_application_stamp"
    ...
    meltbuild_info melt-build-script.tpl:740/613 done check runtime  $meltcheckruntime_stamp
else
    meltbuild_info melt-build-script.tpl:742/614 keeping runtime checks  $meltcheckruntime_stamp
fi

if [ "$melt_overall_goal" = "checkruntime" ]; then
    meltbuild_info melt-build-script.tpl:746/615 done checkruntime overall goal with stamp  $meltcheckruntime_stamp
    exit 0
fi
################################################################
################################################################
#@ melt-build-script.tpl:751/616
if [ "$melt_overall_goal" = "regenerate" ]; then
    meltbuild_info melt-build-script.tpl:753/617 regenerating runtime support
    ...
    echo ' -DGCCMELT_FROM_ARG="melt-build-script.tpl:757/618"' > $meltregen_argstemp
   ...
   meltbuild_info melt-build-script.tpl:767/619 $meltregen_args  is
   cat $meltregen_args < /dev/null > /dev/stderr
    $GCCMELT_CC1_PREFIX $GCCMELT_CC1 @$meltregen_args \
	|| meltbuild_error melt-build-script.tpl:770/620 failed with arguments @$meltregen_args
    meltbuild_info melt-build-script.tpl:771/621 done regenerate overall goal
    exit 0
fi

################################################################
### the generated documentation meltgendoc.texi melt-build-script.tpl:776/622
...
if [   ! -f meltgendoc.texi  -o meltbuild-sources/warmelt-first.melt -nt meltgendoc.texi  \
 ...
 -o meltbuild-sources/xtramelt-opengpu.melt -nt meltgendoc.texi   ]; then
   meltbuild_info melt-build-script.tpl:780/623 generating meltgendoc.texi
   meltgen_args=meltbuild-gendoc.args-tmp$$
   echo ' -DGCCMELT_FROM_ARG="melt-build-script.tpl:782/624"' > $meltgen_args
   ...
   meltbuild_info melt-build-script.tpl:794/625  meltbuild-gendoc.args is
   cat meltbuild-gendoc.args < /dev/null > /dev/stderr
   $GCCMELT_CC1_PREFIX $GCCMELT_CC1 @meltbuild-gendoc.args \
     || meltbuild_error melt-build-script.tpl:797/626 failed with arguments @meltbuild-gendoc.args
else
   meltbuild_info melt-build-script.tpl:799/627 keeping meltgendoc.texi
fi

################
meltbuild_info melt-build-script.tpl:803/628 successfully done with times at `date '+%x %H:%M:%S'`: ; times > /dev/stderr

################################################################
#@ melt-build-script.tpl:806/629
if [ "$melt_overall_goal" = "gendoc" ]; then
    meltbuild_info melt-build-script.tpl:808/630 done gendoc overall goal with stamp  $melt_final_translator_stamp
    exit 0
fi

## #@ melt-build-script.tpl:812/631 if we get here something is wrong in this script
meltbuild_error  melt-build-script.tpl:813/632 unexpected MELT overall goal "$melt_overall_goal" buggy melt-build-script.tpl
#@ eof melt-build-script.tpl:814/633 end of generated melt-build-script.sh
