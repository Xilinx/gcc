...
#@#@# 
#@#@# It has been AutoGen-ed  August 28, 2012 at 02:55:55 PM by AutoGen 5.16
...
if [ ! -f $meltcheckruntime_stamp -o $meltcheckruntime_stamp -ot "$GCCMELT_RUNTIME_ARGS" \
    ...
    [ -f "$meltcheckruntime_args" ] || meltbuild_error  melt-build-script.tpl:681/594 missing check runtime args  "$meltcheckruntime_args"
   meltbuild_info melt-build-script.tpl:682/595 $meltcheckruntime_args  is
   cat $meltcheckruntime_args < /dev/null > /dev/stderr
    $GCCMELT_CC1_PREFIX $GCCMELT_CC1 @$meltcheckruntime_args \
	|| meltbuild_error melt-build-script.tpl:685/596 failed with arguments @$meltcheckruntime_arg
   meltbuild_info melt-build-script.tpl:686/597 done check runtime with $meltcheckruntime_args
   #@ melt-build-script.tpl:687/598 checkhello
    meltcheckhelloworld_args=meltbuild-checkhelloworld.args 
    meltcheckhelloworld_argstemp=$meltcheckhelloworld_args-tmp$$
    echo ' -DGCCMELT_FROM_ARG="melt-build-script.tpl:690/599"' > $meltcheckhelloworld_argstemp
    ...
    [ -f "$meltcheckhelloworld_args" ] || meltbuild_error  melt-build-script.tpl:703/600 missing check helloworld args  "$meltcheckhelloworld_args"
   meltbuild_info melt-build-script.tpl:704/601 $meltcheckhelloworld_args  is
   cat $meltcheckhelloworld_args < /dev/null > /dev/stderr
    $GCCMELT_CC1_PREFIX $GCCMELT_CC1 @$meltcheckhelloworld_args \
	|| meltbuild_error melt-build-script.tpl:707/602 running helloworld failed with arguments @$meltcheckhelloworld_arg
   meltbuild_info melt-build-script.tpl:708/603 done check helloworld with $meltcheckruntime_args
   #@ melt-build-script.tpl:709/604 runtime stamp
    meltcheckruntime_stamptemp=$meltcheckruntime_stamp-tmp$$
    [ -f "$GCCMELT_RUNTIME_C" ] || meltbuild_error melt-build-script.tpl:711/605 missing MELT runtime C file $GCCMELT_RUNTIME_C
    ...
    [ -f "$melt_final_translator_stamp" ] || meltbuild_error melt-build-script.tpl:716/606 missing final translator stamp "$melt_final_translator_stamp"
    [ -f "$melt_final_application_stamp" ] || meltbuild_error melt-build-script.tpl:717/607 missing final application stamp "$melt_final_application_stamp"
    ...
    meltbuild_info melt-build-script.tpl:721/608 done check runtime  $meltcheckruntime_stamp
else
    meltbuild_info melt-build-script.tpl:723/609 keeping runtime checks  $meltcheckruntime_stamp
fi

if [ "$melt_overall_goal" = "checkruntime" ]; then
    meltbuild_info melt-build-script.tpl:727/610 done checkruntime overall goal with stamp  $meltcheckruntime_stamp
    exit 0
fi
################################################################
################################################################
#@ melt-build-script.tpl:732/611
if [ "$melt_overall_goal" = "regenerate" ]; then
    meltbuild_info melt-build-script.tpl:734/612 regenerating runtime support
    ...
    echo ' -DGCCMELT_FROM_ARG="melt-build-script.tpl:738/613"' > $meltregen_argstemp
   ...
   meltbuild_info melt-build-script.tpl:748/614 $meltregen_args  is
   cat $meltregen_args < /dev/null > /dev/stderr
    $GCCMELT_CC1_PREFIX $GCCMELT_CC1 @$meltregen_args \
	|| meltbuild_error melt-build-script.tpl:751/615 failed with arguments @$meltregen_args
    meltbuild_info melt-build-script.tpl:752/616 done regenerate overall goal
    exit 0
fi

################################################################
### the generated documentation meltgendoc.texi melt-build-script.tpl:757/617
...
if [   ! -f meltgendoc.texi  -o meltbuild-sources/warmelt-first.melt -nt meltgendoc.texi  \
 ...
 -o meltbuild-sources/xtramelt-opengpu.melt -nt meltgendoc.texi   ]; then
   meltbuild_info melt-build-script.tpl:761/618 generating meltgendoc.texi
   meltgen_args=meltbuild-gendoc.args-tmp$$
   echo ' -DGCCMELT_FROM_ARG="melt-build-script.tpl:763/619"' > $meltgen_args
   ...
   meltbuild_info melt-build-script.tpl:775/620  meltbuild-gendoc.args is
   cat meltbuild-gendoc.args < /dev/null > /dev/stderr
   $GCCMELT_CC1_PREFIX $GCCMELT_CC1 @meltbuild-gendoc.args \
     || meltbuild_error melt-build-script.tpl:778/621 failed with arguments @meltbuild-gendoc.args
else
   meltbuild_info melt-build-script.tpl:780/622 keeping meltgendoc.texi
fi

################
meltbuild_info melt-build-script.tpl:784/623 successfully done with times at `date '+%x %H:%M:%S'`: ; times > /dev/stderr

################################################################
#@ melt-build-script.tpl:787/624
if [ "$melt_overall_goal" = "gendoc" ]; then
    meltbuild_info melt-build-script.tpl:789/625 done gendoc overall goal with stamp  $melt_final_translator_stamp
    exit 0
fi

## #@ melt-build-script.tpl:793/626 if we get here something is wrong in this script
meltbuild_error  melt-build-script.tpl:794/627 unexpected MELT overall goal "$melt_overall_goal" buggy melt-build-script.tpl
#@ eof melt-build-script.tpl:795/628 end of generated melt-build-script.sh
