...
#@#@# 
#@#@# It has been AutoGen-ed  August 28, 2012 at 09:36:00 PM by AutoGen 5.12
...
if [ ! -f $meltcheckruntime_stamp -o $meltcheckruntime_stamp -ot "$GCCMELT_RUNTIME_ARGS" \
   ...
   cat $meltcheckruntime_args < /dev/null > /dev/stderr
	...
	|| meltbuild_error melt-build-script.tpl:686/596 failed with arguments @$meltcheckruntime_arg
   meltbuild_info melt-build-script.tpl:687/597 done check runtime with $meltcheckruntime_args
   #@ melt-build-script.tpl:688/598 checkhello
    meltcheckhelloworld_args=meltbuild-checkhelloworld.args 
    meltcheckhelloworld_argstemp=$meltcheckhelloworld_args-tmp$$
    echo ' -DGCCMELT_FROM_ARG="melt-build-script.tpl:691/599"' > $meltcheckhelloworld_argstemp
    ...
    [ -f "$meltcheckhelloworld_args" ] || meltbuild_error  melt-build-script.tpl:705/600 missing check helloworld args  "$meltcheckhelloworld_args"
   meltbuild_info melt-build-script.tpl:706/601 $meltcheckhelloworld_args  is
   cat $meltcheckhelloworld_args < /dev/null > /dev/stderr
    $GCCMELT_CC1_PREFIX $GCCMELT_CC1 @$meltcheckhelloworld_args \
	|| meltbuild_error melt-build-script.tpl:709/602 running helloworld failed with arguments @$meltcheckhelloworld_args
   meltbuild_info melt-build-script.tpl:710/603 done check helloworld with $meltcheckhelloworld_args
   #@ melt-build-script.tpl:711/604 runtime stamp
    meltcheckruntime_stamptemp=$meltcheckruntime_stamp-tmp$$
    [ -f "$GCCMELT_RUNTIME_C" ] || meltbuild_error melt-build-script.tpl:713/605 missing MELT runtime C file $GCCMELT_RUNTIME_C
    ...
    [ -f "$melt_final_translator_stamp" ] || meltbuild_error melt-build-script.tpl:718/606 missing final translator stamp "$melt_final_translator_stamp"
    [ -f "$melt_final_application_stamp" ] || meltbuild_error melt-build-script.tpl:719/607 missing final application stamp "$melt_final_application_stamp"
    ...
    meltbuild_info melt-build-script.tpl:723/608 done check runtime  $meltcheckruntime_stamp
else
    meltbuild_info melt-build-script.tpl:725/609 keeping runtime checks  $meltcheckruntime_stamp
fi

if [ "$melt_overall_goal" = "checkruntime" ]; then
    meltbuild_info melt-build-script.tpl:729/610 done checkruntime overall goal with stamp  $meltcheckruntime_stamp
    exit 0
fi
################################################################
################################################################
#@ melt-build-script.tpl:734/611
if [ "$melt_overall_goal" = "regenerate" ]; then
    meltbuild_info melt-build-script.tpl:736/612 regenerating runtime support
    ...
    echo ' -DGCCMELT_FROM_ARG="melt-build-script.tpl:740/613"' > $meltregen_argstemp
   ...
   meltbuild_info melt-build-script.tpl:750/614 $meltregen_args  is
   cat $meltregen_args < /dev/null > /dev/stderr
    $GCCMELT_CC1_PREFIX $GCCMELT_CC1 @$meltregen_args \
	|| meltbuild_error melt-build-script.tpl:753/615 failed with arguments @$meltregen_args
    meltbuild_info melt-build-script.tpl:754/616 done regenerate overall goal
    exit 0
fi

################################################################
### the generated documentation meltgendoc.texi melt-build-script.tpl:759/617
...
if [   ! -f meltgendoc.texi  -o meltbuild-sources/warmelt-first.melt -nt meltgendoc.texi  \
 ...
 -o meltbuild-sources/xtramelt-opengpu.melt -nt meltgendoc.texi   ]; then
   meltbuild_info melt-build-script.tpl:763/618 generating meltgendoc.texi
   meltgen_args=meltbuild-gendoc.args-tmp$$
   echo ' -DGCCMELT_FROM_ARG="melt-build-script.tpl:765/619"' > $meltgen_args
   ...
   meltbuild_info melt-build-script.tpl:777/620  meltbuild-gendoc.args is
   cat meltbuild-gendoc.args < /dev/null > /dev/stderr
   $GCCMELT_CC1_PREFIX $GCCMELT_CC1 @meltbuild-gendoc.args \
     || meltbuild_error melt-build-script.tpl:780/621 failed with arguments @meltbuild-gendoc.args
else
   meltbuild_info melt-build-script.tpl:782/622 keeping meltgendoc.texi
fi

################
meltbuild_info melt-build-script.tpl:786/623 successfully done with times at `date '+%x %H:%M:%S'`: ; times > /dev/stderr

################################################################
#@ melt-build-script.tpl:789/624
if [ "$melt_overall_goal" = "gendoc" ]; then
    meltbuild_info melt-build-script.tpl:791/625 done gendoc overall goal with stamp  $melt_final_translator_stamp
    exit 0
fi

## #@ melt-build-script.tpl:795/626 if we get here something is wrong in this script
meltbuild_error  melt-build-script.tpl:796/627 unexpected MELT overall goal "$melt_overall_goal" buggy melt-build-script.tpl
#@ eof melt-build-script.tpl:797/628 end of generated melt-build-script.sh
