...
;; Since constant address in x32 is signed extended to 64bit, we have
;; to convert negative address to positive one.
(define_split
  [(set (mem:SWI1248x (match_operand:DI 0 "const_int_operand" ""))
	(match_operand:SWI1248x 1 "register_operand" ""))]
  "TARGET_X32 && reload_completed && INTVAL (operands[0]) < 0"
  [(set (match_dup 0) (match_dup 1))]
{
  rtx addr = GEN_INT ((unsigned int) INTVAL (operands[0]));
  operands[0] = gen_rtx_MEM (<MODE>mode, addr);
})

(define_split
  [(set (match_operand:SWI1248x 0 "register_operand" "")
        (mem:SWI1248x (match_operand:DI 1 "const_int_operand" "")))]
  "TARGET_X32 && reload_completed && INTVAL (operands[1]) < 0"
  [(set (match_dup 0) (match_dup 1))]
{
  rtx addr = GEN_INT ((unsigned int) INTVAL (operands[1]));
  operands[1] = gen_rtx_MEM (<MODE>mode, addr);
})
