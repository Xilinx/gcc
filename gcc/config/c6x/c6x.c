		     ...
		     insn_rr_info *info, unsigned int op_mask, int orig_side)
{
  ...
  vec<du_head_p> involved_chains = NULL;
...
hwloop_optimize (hwloop_info loop)
{
  ...
  entry_edge = NULL;
  FOR_EACH_VEC_ELT (loop->incoming, i, entry_edge)
  ...
  if (!single_succ_p (entry_bb) || loop->incoming.length () > 1)
    {
      basic_block new_bb;
      edge e;
      edge_iterator ei;
