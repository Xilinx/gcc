  ...
  if (IS_STACKALIGN (func_type))
    {
	 ...
	 Just tell it we saved SP in r0.  */
      ...
      /* ??? The CFA changes here, which may cause GDB to conclude
	 that it has entered a different function.  That said, the
	 unwind info is correct before and after this instruction
	 because we've described the save of SP, which will override
	 the default handling of SP as restoring from the CFA.  */
