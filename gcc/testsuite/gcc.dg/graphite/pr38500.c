...
extern char test1;
Perl_re_intuit_start( int minlen,  char *strend, unsigned int flags, int i, unsigned int  sv_flags)
{
    ...
    if (end_shift < eshift)
           end_shift = eshift;
    restart:
           s = (char *) (Perl_screaminstr(sv_flags, start_shift + (s - strbeg), end_shift, 0));
