int main (int argc, char **argv)
{
  ...
  while (array2[:][:]) /* { dg-error "array notations cannot be used as a condition in a while statement" } */
    x = 3;

  while (array[1:1:1]) /* { dg-error "array notations cannot be used as a condition in a while statement" } */
    x = 1;

  while (ii != array2[1:x:3][1:2:1]) /* { dg-error "array notations cannot be used as a condition in a while statement"  } */
    x = 2;

  do {
    x = 3;
  } while (ii != array2[:][:]); /* { dg-error "array notations cannot be used as a condition for a do-while statement" } */
    x = 3;

  do { 
    x = 2;
  } while (ii != (x + array2[:][1:x:2]) + 2); /* { dg-error "array notations cannot be used as a condition for a do-while statement" } */
