...
cilk_for (vector<int>::reverse_iterator iter4 = array_serial.rbegin(); iter4 != array_serial.rend(); iter4++)
{
  if (*iter4 == 0x8) {
    *iter4 = 9;
  }
...
for (vector<int>::reverse_iterator iter2 = array_serial.rbegin(); iter2 != array_serial.rend();
          iter2 += 1) 
{
   if ((*iter2 == 0x4) || (*iter2 == 0x7)) {
    *iter2 = 0x3;
