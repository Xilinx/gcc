...
static void
init_perm2i128 (unsigned long long *src1,
		...
		unsigned long long *dst)
{
  ...
  switch (temp) {
  case 0: memcpy (dst, src1, 16);
  case 1: memcpy (dst, src1+2, 16);
  case 2: memcpy (dst, src2, 16);
  case 3: memcpy (dst, src1+2, 16);
  }

  temp = (mask>>4) & 3;

  switch (temp) {
  case 0: memcpy (dst+2, src1, 16);
  case 1: memcpy (dst+2, src1+2, 16);
  case 2: memcpy (dst+2, src2, 16);
  case 3: memcpy (dst+2, src1+2, 16);
  }
