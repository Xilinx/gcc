...
#define ggc_alloc_cleared_vec_entrytreemelt_st(Len) \
  ((struct entrytreemelt_st*)ggc_alloc_cleared ((Len)*sizeof(struct entrytreemelt_st)))
#define ggc_alloc_vec_entrytreemelt_st(Len) \
  ((struct entrytreemelt_st*)ggc_alloc ((Len)*sizeof(struct entrytreemelt_st)))
...
#define MAKECHDIR_ARG "-C"
  ...
  if (fullbinfilelen<4
      ...
      || fullbinfile[fullbinfilelen-2] != 's'
      || fullbinfile[fullbinfilelen-1] != 'o')
    melt_fatal_error ("invalid MELT module binary file %s (not a .so)",
		      fullbinfile);
...
/* eof $Id: melt-runtime.c 168746 2011-01-13 13:39:28Z bstarynk $ */
