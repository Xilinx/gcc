  ...
  *error_msg = NULL;
  ...
  if ((TREE_CODE (decl) == CONST_DECL && DECL_TEMPLATE_PARM_P (decl))
	  ...
	  && (wrap = get_tls_wrapper_fn (decl)))
	{
	  ...
	  decl = build_cxx_call (wrap, spawning, 0, NULL, tf_warning_or_error);
	}
      else if (scope)
	{
...
maybe_constant_value (tree t)
{
  ...
  if (flag_enable_cilk && contains_array_notation_expr (t))
    return t;
...
maybe_add_lambda_conv_op (tree type)
{
  ...
  call = build_call_a (callop, CILK_CALL_NORMAL, argvec->length (), 
		       argvec->address ());
