...
(cp_parser *, bool, bool, bool, enum call_context, cp_id_kind *);
...
(cp_parser *, struct pragma_simd_values *);
...
(cp_parser *, tree, int);
static tree cp_parser_cilk_for_init_statement
(cp_parser *, tree *);
static void cp_parser_cilk_grainsize
(cp_parser *, cp_token *);

static tree cp_parser_cilk_for_condition(cp_parser *parser);
static tree cp_parser_cilk_for_expression_iterator(cp_parser *parser);


static void cp_parser_simd_vectorlength (cp_parser *parser,
					 cp_token *pragma_token);
static void cp_parser_simd_private (cp_parser *parser, cp_token *pragma_token);
static void cp_parser_simd_reduction (cp_parser *parser,
				      cp_token *pragma_token);
static void cp_parser_simd_linear (cp_parser *parser, cp_token *pragma_token);
static void cp_parser_simd_assert (cp_parser *parser,
				   cp_token *pragma_token, bool is_assert);
			      ...
			      cp_id_kind * pidk_return)
{
  ...
  case RID_CILK_SPAWN:
  {
    /* If the spawn keyword is seen this must be a function call
       (XXX or a function-call like constructor invocation).
       Set the flag and execute the default case without the
       speculative processing of non-call expressions. */
    cp_lexer_consume_token (parser->lexer);
    token = cp_lexer_peek_token (parser->lexer);
    if (token->type == CPP_SEMICOLON)
    {
      /* Nicer error than "expected primary-expression before ;" */
      error ("%<cilk_spawn%> must be followed by expression");
    ...
    {
      DECL_STRUCT_FUNCTION (current_function_decl)->calls_spawn = 1;
      SPAWN_CALL_P(current_function_decl)=1;
      DECL_STATIC_CHAIN (current_function_decl) = 1; /* we do oppo. here */
      begin_cilk_block ();
      gcc_assert (spawning == CALL_NORMAL);
      spawning = CALL_SPAWN;
      /* bviyer: I am just trying this: VERIFY!! */
			    ...
			    cp_id_kind * pidk)
{
  ...
  if (unary_operator == ERROR_MARK)
  {
	     ...
	     && token->type == CPP_AND_AND)
    {
      ...
      /* Hide any labels outside an active Cilk block */
      if (parser->in_statement & (IN_CILK_FOR|IN_CILK_SPAWN))
        (void) cilk_block_local_label (identifier);
...
static tree
cp_parser_for (cp_parser *parser, struct pragma_simd_values *ps_values)
{
  ...
  if (is_range_for)
    statement = cp_parser_range_for (parser, scope, init, decl);
  else
    statement = cp_parser_c_for (parser, scope, init);

  /* bviyer: I moved all the for staement computation into this 
     function, so that I can do the pragma simd computations 
  */
  /* Look for the `)'.  */
  cp_parser_require (parser, CPP_CLOSE_PAREN, RT_CLOSE_PAREN);
  ...
  if (ps_values != NULL)
  {
    FOR_STMT_PRAGMA_SIMD_INDEX(statement) = psv_head_insert (*ps_values);
...
cp_parser_iteration_statement (cp_parser* parser)
{
  ...
  case RID_FOR:
  {
    ...
    statement = cp_parser_for (parser, NULL);
    /* bviyer: I moved all the for staement computation into 
       cp_parser_for function, so that I can do the 
       pragma simd computations 
    */
...
cp_parser_jump_statement (cp_parser* parser)
{
  ...
  case RID_BREAK:
    ...
    case IN_CILK_SPAWN:
      ...
      break;
    case IN_CILK_FOR:
      error ("break statement used with Cilk for loop");
      break;
  ...
  case RID_RETURN:
  {
    ...
    if (parser->in_statement & IN_CILK_FOR)
    {
      error ("return statement used in Cilk for loop");
      TREE_NO_WARNING (statement) = 1;
    }
  ...
  case RID_GOTO:
    ...
    {
      tree dest = cp_parser_identifier(parser);
...
#if 0
/* parse a for statement: This function is only used after we hit a
 *  a pragma simd
 */
static void
cp_parser_for_statement (cp_parser *parser,
                         struct pragma_simd_values *ps_values)
{
  tree statement = NULL_TREE;
  unsigned char in_statement = parser->in_statement;
...
/* bviyer: this function is a replica of the above
 * cp_parser_cilk_init_declarator, only difference is that in this function
 * the intializer value is returned. WE need this information when we are
 * gimplifying the cilk_for
 */

static tree
cp_parser_cilk_init_declarator (cp_parser* parser,
				cp_decl_specifier_seq *decl_specifiers,
				VEC (deferred_access_check,gc)* checks,
				tree *initializer,
				bool function_definition_allowed_p,
...
cp_parser_initializer_list (cp_parser* parser, bool* non_constant_p)
{
  ...
  *non_constant_p = false;
  ...
  while (true)
  {
	...
	{
	  cp_lexer_consume_token (parser->lexer);
	  identifier = cp_parser_constant_expression (parser, false, NULL);
	  cp_parser_require (parser, CPP_CLOSE_SQUARE, RT_CLOSE_SQUARE);
	  cp_parser_require (parser, CPP_EQ, RT_EQ);
	}
    else
...
cp_parser_pragma (cp_parser *parser, enum pragma_context context)
{
  ...
  case PRAGMA_CILK_GRAINSIZE:
    if (context == pragma_external)
    {
      error ("%<#pragma cilk grainsize%> may only be used inside a function");
      break;
    }
...
/* bviyer: Here we are all the cilk rountines */

static tree
cp_parser_cilk_for (cp_parser *parser, tree grain, int in_statement)
{
  tree statement=NULL_TREE,decl=NULL_TREE,condition=NULL_TREE;
  tree expression=NULL_TREE;
  bool valid = false;
  tree init = NULL_TREE;
  extern void finish_cilk_for_init_stmt(tree);
  extern void finish_cilk_for_cond(tree,tree);
  extern void cilk_erase_for(tree);
  extern bool cilk_validate_for (tree);

  /* now we begin the for statement */
  statement=begin_cilk_for_stmt();

  /* bviyer: I made the change below */
  cp_parser_require(parser, CPP_OPEN_PAREN,/* "'('" */ RT_OPEN_PAREN); /* look for '(' */

  /* valid=feature_requires_cilk ("cilk_for"); */
  valid = true;
  ...
  /* now we parse the declaration */
  decl = cp_parser_cilk_for_init_statement(parser, &init);
  if (decl==error_mark_node)
  {
    valid=false;
  }
  else if (TREE_CODE(decl) != VAR_DECL) /* if the first init is not var decl */
  {
    error("The Cilk initializer does not declare a variable.");
    decl=error_mark_node;
    valid=false;
  }
	   ...
	   (TYPE_NEEDS_CONSTRUCTING(TREE_TYPE(decl)) == 0))
  {
     ...
     * lower-bound
     */
    error ("Cilk Control variable of the for loop needs to be intialized.");
    valid=false;
  }
...
/*  TREE_TYPE(init) = TREE_TYPE(decl); */
  ...
  if (cp_lexer_next_token_is(parser->lexer, CPP_COMMA))
  {
    error("Cilk for loop initializer must have ONLY 1 variable declaration");
    cp_parser_skip_to_end_of_statement(parser);
  }
  ...
  if (cp_lexer_next_token_is(parser->lexer,CPP_SEMICOLON))
  {
    error("Cilk for loop requires a condition.");
  ...
  /* now we handle the for condition */
  finish_cilk_for_cond(condition,statement);

  /* now we look for a ; after the for condition */
  cp_parser_consume_semicolon_at_end_of_statement(parser);

  /* check to see an expr is present, else complain.*/
  if (cp_lexer_next_token_is(parser->lexer,CPP_CLOSE_PAREN))
  {
    error("Cilk for loop requires increment statement.");
    expression=error_mark_node;
    valid=false;
  }
  else
  {
    expression=cp_parser_cilk_for_expression_iterator(parser);
  ...
  /* we just finish it like a normal for expr */
  finish_for_expr(expression, statement);

  /* make sure we have a ')' */
  /* bviyer: I made this change */
  cp_parser_require(parser, CPP_CLOSE_PAREN,/* "')'" */ RT_CLOSE_PAREN); 

  parser->in_statement=IN_CILK_FOR;
  cp_parser_already_scoped_statement(parser);
  parser->in_statement=in_statement;
...
static tree cp_parser_cilk_for_condition (cp_parser *parser)
{
  ...
  lhs=cp_parser_binary_expression(parser, false, false, PREC_SHIFT_EXPRESSION,
				  NULL);
  token=cp_lexer_peek_token(parser->lexer);

  if (token->type==CPP_NOT_EQ)
  {
    code=NE_EXPR;
  }
  ...
  else if (token->type==CPP_LESS_EQ)
  {
    code=LE_EXPR;
  }
  else if (token->type==CPP_GREATER_EQ)
  {
    code=GE_EXPR;
  }
  else if (token->type==CPP_GREATER)
  {
    code=GT_EXPR;
  }
  else if (token->type==CPP_EQ_EQ)
  {
    error("equality test not permitted i the Cilk for loop.\n");
    code=ERROR_MARK;
  }
  else
  {
    error("Missing comparison operator in the loop condition.\n");
    code=ERROR_MARK;
  }
...
static tree cp_parser_cilk_for_expression_iterator(cp_parser *parser)
{
  ...
  name=cp_parser_lookup_name(parser,token->u.value,none_type,false,false,false,
			     NULL, token->location);
  if (name==error_mark_node)
  {
    return error_mark_node;
  ...
  if (t_code != NOP_EXPR)
  {
    if (token->type != CPP_CLOSE_PAREN)
    {
      /* this means there are token after var. What we are trying to catch
  ...
  if (token->type == CPP_CLOSE_PAREN)
  {
    /* this means the control variable is not changed. We are trying to catch
     * the following case:
     * for (ii = 0; ii < 10; ii)
     */
  ...
  cp_lexer_consume_token(parser->lexer);
  /* new_token=cp_lexer_peek_token(parser->lexer); */

  if (token->type==CPP_PLUS_PLUS)
  {
    return build2(POSTINCREMENT_EXPR, void_type_node, name, NULL_TREE);
  }
  else if (token->type==CPP_MINUS_MINUS)
  {
    return build2(POSTDECREMENT_EXPR, void_type_node, name, NULL_TREE);
  }
  else if (token->type==CPP_EQ)
  {
    sorry("Cilk for loop increment with = operator.");
    return error_mark_node;
  }
  else if (token->type==CPP_PLUS_EQ)
  {
    t_code=PLUS_EXPR;
  }
  else if (token->type==CPP_MINUS_EQ)
  {
    t_code=MINUS_EXPR;
  }
  else if ((token->type==CPP_MOD_EQ)    ||
	   (token->type==CPP_XOR_EQ)    ||
	   (token->type==CPP_DIV_EQ)    ||
	   (token->type==CPP_AND_EQ)    ||
	   (token->type==CPP_OR_EQ)     ||
	   (token->type==CPP_MULT_EQ)   ||
	   (token->type==CPP_LSHIFT_EQ) ||
  ...
  expr=cp_parser_binary_expression(parser,false, false, PREC_NOT_OPERATOR,NULL);
  if (expr==error_mark_node)
  {
    return expr;
  }
...
cp_parser_simd_vectorlength (cp_parser *parser, cp_token *pragma_token)
{
  ...
  local_simd_values.types |= P_SIMD_VECTORLENGTH;
      ...
      (cp_lexer_next_token_is (parser->lexer, CPP_KEYWORD)))
  {
    ...
    else if (strcmp (IDENTIFIER_POINTER(token->u.value), "vectorlength") == 0)
...
cp_parser_simd_private (cp_parser *parser, cp_token *pragma_token)
{
  ...
  if (cp_parser_require (parser, CPP_OPEN_PAREN, RT_OPEN_PAREN))
  {
	...
	else
	{
	  cp_parser_error (parser, "expected variable");
	  cp_parser_skip_to_pragma_eol (parser, pragma_token);
	  exit(-1); /* you exit here because this shouldn't happen. If it does
...
static void
cp_parser_simd_assert(cp_parser *parser, cp_token *pragma_token, bool is_assert)
{
  cp_token *token;

  if ((local_simd_values.types == P_SIMD_ASSERT) &&
      (is_assert == false))
  {
...
static void
cp_parser_cilk_grainsize (cp_parser *parser, cp_token *pragma_token)
{
...
cp_parser_cilk_for_init_statement(cp_parser *parser, tree *init)
{
  ...
  if (token->type == CPP_NAME)
  {
    type=cp_parser_lookup_name_simple(parser, token->u.value, token->location);
  ...
  if (specs.storage_class != sc_none)
  {
    ...
    error("Control variable of Cilk may not have explicit storage class.\n");
    return error_mark_node;
  }
  ...
  else if (specs.type == error_mark_node)
  {
    cp_parser_skip_to_end_of_statement(parser);
    return error_mark_node;
  }

  decl = cp_parser_cilk_init_declarator(parser, &specs, NULL, init,
					false, false, flags,
					NULL);
