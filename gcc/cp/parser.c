...
static void cp_parser_cilk_grainsize
  (cp_parser *, cp_token *);
static tree cp_parser_cilk_for_condition
  (cp_parser *parser);
...
cp_parser_primary_expression (cp_parser *parser,
			      ...
			      bool cast_p,
			      bool template_arg_p,
			      enum call_context context,
			      cp_id_kind *idk)
{
  return cp_parser_primary_expression (parser, address_p, cast_p, 
				       template_arg_p, context,
				       /*decltype*/false, idk);
}
...
static tree
cp_parser_for (cp_parser *parser, struct pragma_simd_values *cilkplus_ps_values)
{
  ...
  /* bviyer: I moved all the for statement computation into this 
...
cp_parser_iteration_statement (cp_parser* parser)
{
  ...
  switch (keyword)
	...
	/* bviyer: I moved all the statement computation into cp_parser_for
	   function so that I can do the pragma_simd work */
      }
      break;
...
/* Here we start all the Cilk Plus rountines */

/* This function is the entry function of Cilk_for parser.  */
static tree
cp_parser_cilk_for (cp_parser *parser, tree grain, int in_statement)
{
  ...
  if (decl == error_mark_node) 
    valid = false;
  else if (TREE_CODE (decl) != VAR_DECL) /* if first init is not var decl */
    {
  ...
  if (cp_lexer_next_token_is (parser->lexer, CPP_COMMA))
    {
      error ("Cilk for loop initializer can have ONLY 1 variable declaration");
      cp_parser_skip_to_end_of_statement (parser);
    }
  ...
  if (!valid || (!processing_template_decl && !cilk_validate_for (statement)))
    {
...
/* This function will parse the condition part of the Cilk_for statement.  */
...
/* This function parses the iterator of the cilk_for statement.  */
...
/* this function will handle the pragma cilk grainsize. The syntax is:
...
cp_parser_cilk_grainsize (cp_parser *parser, cp_token *pragma_token)
{
  ...
  if (!flag_enable_cilk)
    {
      warning (0, "%<#pragma cilk grainsize not recognized without -fcilkplus");
      return;
    }
  if (cp_parser_require (parser, CPP_EQ, RT_EQ))
    {
      ...
      if (exp && (exp != error_mark_node))
	{
	  ...
	  /* Make sure the pragma is right before a cilk_for.  */
...
/* This function will parse the initializer of the cilk_for statement.  */
  ...
  if (specs.storage_class != sc_none)
    {
      ...
      error ("Control variable of Cilk cannot have explicit storage class.\n");
      return error_mark_node;
    }
...
{
  vec<tree, va_gc> *proc_vec_list = NULL;
  ...
  if (cp_lexer_next_token_is_not (parser->lexer, CPP_CLOSE_PAREN))
	...
	vec_safe_push (proc_vec_list, build_string (strlen ("pentium_4_sse3"),
				     "pentium_4_sse3"));
      else if (token->u.value
	       && TREE_CODE (token->u.value) == IDENTIFIER_NODE
	       && simple_cst_equal (token->u.value,
...
/* This function handles the uniform clause in elemental function attribute.  */
...
cp_parser_elem_fn_uniform_clause (cp_parser *parser)
{
  ...
  while (cp_lexer_next_token_is_not (parser->lexer, CPP_CLOSE_PAREN))
      ...
      if (token->u.value && token->type == CPP_NAME)
	{
	  ...
	  vec_safe_push (uniform_vec, str_token);
	  if (cp_lexer_next_token_is (parser->lexer, CPP_COMMA))
	    {
	      cp_lexer_consume_token (parser->lexer);
...
/* This function handles vectorlength clause in elemental func. attribute.  */

static tree
cp_parser_elem_fn_vlength_clause (cp_parser *parser)
{
  vec<tree, va_gc> *vlength_vec = NULL;
  ...
  if (cp_lexer_next_token_is_not (parser->lexer, CPP_CLOSE_PAREN))
	...
	vec_safe_push (vlength_vec, token->u.value);
      if (cp_lexer_next_token_is_not (parser->lexer, CPP_CLOSE_PAREN))
	{
...
/* This function handles the linear clause in elemental function attribute.  */

static tree
cp_parser_elem_fn_linear_clause (cp_parser *parser)
{
  vec<tree,va_gc> *linear_vec = NULL;
  ...
  while (cp_lexer_next_token_is_not (parser->lexer, CPP_CLOSE_PAREN))
      ...
      if (token->u.value && token->type == CPP_NAME)
	{
	  ...
	  vec_safe_push (linear_vec, var_str);
	  vec_safe_push (linear_vec, step_size);
	  if (cp_lexer_next_token_is (parser->lexer, CPP_COMMA))
	    {
	      cp_lexer_consume_token (parser->lexer);
	      if (cp_lexer_next_token_is_not (parser->lexer, CPP_NAME))
...
cp_parser_elem_fn_expression_list (cp_parser *parser)
{
  ...
  if (proc_list)
    vec_safe_push (expr_list, proc_list);
  if (vlength_list)
    vec_safe_push (expr_list, vlength_list);
  if (uniform_list)
    vec_safe_push (expr_list, uniform_list);
  if (mask_list)
    vec_safe_push (expr_list, mask_list);
  if (linear_list)
    vec_safe_push (expr_list, linear_list);
  
  return expr_list;
}

/* All Cilk Plus related parsing functions end here.  */
